...
import { createDraftRequisition, validateRequisition, fetchProducts } from '@renderer/app/store/slice/inventorySlice';
// import { UIMedication } from '../types';
    ...
    return (
        <div className="fixed inset-0 z-[100] bg-slate-900/80 backdrop-blur-xl flex items-center justify-center p-4">
            <div className="bg-white dark:bg-slate-900 w-full max-w-lg rounded-[2rem] p-8 shadow-2xl border border-slate-200 dark:border-slate-800">
                <h3 className="text-xl font-black text-slate-900 dark:text-white mb-1">Nouvelle Livraison</h3>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="relative">
                        <label className="text-[10px] uppercase font-bold text-slate-500 ml-1">Produit à approvisionner</label>
                        <input
                            type="text"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1">
                            <label className="text-[10px] uppercase font-bold text-slate-500 ml-1">Quantité Reçue</label>
                            <input type="number" className="w-full p-4 bg-slate-50 dark:bg-slate-800 rounded-xl outline-none font-bold dark:text-white" value={quantity} onChange={e => setQuantity(Number(e.target.value))} required min="1" />
                        </div>
                        <div className="space-y-1">
                            <label className="text-[10px] uppercase font-bold text-slate-500 ml-1">Prix Achat Lot</label>
                            <input type="number" className="w-full p-4 bg-slate-50 dark:bg-slate-800 rounded-xl outline-none font-bold dark:text-white" value={buyingPrice} onChange={e => setBuyingPrice(Number(e.target.value))} required />
                        </div>
                    </div>

                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1">
                            <label className="text-[10px] uppercase font-bold text-slate-500 ml-1">Numéro Lot</label>
                            <input type="text" placeholder="BATCH-001" className="w-full p-4 bg-slate-50 dark:bg-slate-800 rounded-xl outline-none font-bold dark:text-white" value={batchNumber} onChange={e => setBatchNumber(e.target.value)} required />
                        </div>
                        <div className="space-y-1">
                            <label className="text-[10px] uppercase font-bold text-slate-500 ml-1">Date Péremption</label>
                            <input type="date" className="w-full p-4 bg-slate-50 dark:bg-slate-800 rounded-xl outline-none font-bold dark:text-white" value={expiryDate} onChange={e => setExpiryDate(e.target.value)} required />
                        </div>
                    </div>

                    <div className="flex gap-3 pt-4">
                        <button type="button" onClick={onClose} className="flex-1 py-4 bg-slate-100 dark:bg-slate-800 text-slate-500 font-bold rounded-xl">Annuler</button>
                        <button type="submit" disabled={isSubmitting || !selectedProductId} className="flex-1 py-4 bg-emerald-600 text-white font-bold rounded-xl hover:bg-emerald-500 shadow-lg shadow-emerald-600/20 disabled:opacity-50">
