...
export const RequisitionEditor: React.FC<Props> = ({ onClose, onSuccess }) => {
    ...
    const updateLine = (id: string, field: keyof LineItem, value: any) => {
        setItems(items.map(item => {
            if (item.id === id) {
                // Si on change le produit, on pré-remplit le prix d'achat
                if (field === 'productId') {
                    const prod = products.find(p => p.id === value);
                    return { ...item, [field]: value, buyPrice: prod?.buyingPrice || 0 };
                }
    ...
    const handleSaveDraft = async () => {
        ...
        try {
            await dispatch(createDraftRequisition({
                supplierId,
                createdById: user!.id,
    ...
    return (
        <div className="fixed inset-0 z-[100] bg-slate-900/95 backdrop-blur-xl flex flex-col p-4 md:p-8 animate-in fade-in">
            ...
            <div className="flex justify-between items-center mb-8">
                <div>
                    <h2 className="text-2xl font-black text-white uppercase tracking-tight">Nouvelle Réquisition</h2>
                    <p className="text-slate-400 text-xs mt-1">Création de lot en masse (Mode Brouillon)</p>
                </div>
                <div className="flex gap-3">
                    <button onClick={onClose} className="px-6 py-3 bg-slate-800 text-white font-bold rounded-xl hover:bg-slate-700">Fermer</button>
                    <button onClick={handleSaveDraft} className="px-6 py-3 bg-sky-600 text-white font-bold rounded-xl hover:bg-sky-500 shadow-lg shadow-sky-600/20">Sauvegarder Brouillon</button>
                </div>
            </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 bg-white dark:bg-slate-900 rounded-3xl border border-slate-700 overflow-hidden shadow-2xl">
                <div className="overflow-auto h-full custom-scrollbar">
                            ...
                            {items.map((item, index) => (
                                <tr key={item.id} className="hover:bg-slate-800/50 transition-colors group">
                                    ...
                                    <td className="p-4">
                                        ...
                                        >
                                            <option value="" className="bg-slate-900">Sélectionner un produit...</option>
                                            {products.map(p => (
                                    ...
                                    <td className="p-4 text-center">
                                        <button onClick={() => removeLine(item.id)} className="text-red-500 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity">
                                            ✕
                                        </button>
                                    </td>
                                </tr>
                    ...
                    <button onClick={addLine} className="w-full py-4 text-slate-500 hover:text-sky-400 hover:bg-slate-800/30 transition-all font-bold text-xs uppercase tracking-widest border-t border-dashed border-slate-700">
                        + Ajouter une ligne
                    </button>
                </div>
            </div>
