...
const MOCK_DOCS: BillingDocument[] = [
    {
        ...
        items: [
            { id: '1', name: 'Paracétamol', code: 'P001', category: 'Analgésique', dosage: '500mg', price: 1500, quantity: 2, buyingPrice: 1000, threshold: 10, lots: [], selectedLotId: 'L1', selectedLotBatch: 'L2401', selectedLotExpiry: '2025-12-01' },
            { id: '4', name: 'Vitamine C', code: 'V004', category: 'Supplément', dosage: '1000mg', price: 5000, quantity: 1, buyingPrice: 3500, threshold: 20, lots: [], selectedLotId: 'L4', selectedLotBatch: 'L2404', selectedLotExpiry: '2026-01-10' }
        ],
        subtotal: 8000,
        discountAmount: 0,
        total: 8000,
    ...
    { id: 'FAC-2024-1023', type: DocumentType.INVOICE, date: '2024-03-22', customerName: 'Comptoir', items: [], subtotal: 4500, discountAmount: 0, total: 4500, status: 'CANCELLED' },
    { id: 'FAC-2024-1024', type: DocumentType.INVOICE, date: '2024-03-23', customerName: 'Michel Mwamba', items: [], subtotal: 85000, discountAmount: 0, total: 85000, status: 'PENDING', paymentMethod: 'CREDIT' },
];
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-700">
            <div className="flex flex-col md:flex-row gap-6 justify-between items-end no-print">
                <div>
                    <h2 className="text-4xl font-black italic text-slate-900 dark:text-white tracking-tighter uppercase leading-none">Gestion Financière</h2>
                    <p className="text-slate-400 font-bold text-[10px] uppercase tracking-[0.3em] mt-3">Ledger MariaSaas • Flux de Trésorerie GxP</p>
                </div>
                ...
                <div className="flex bg-white dark:bg-slate-900 p-1.5 rounded-[1.8rem] border border-slate-200 dark:border-slate-800 shadow-sm shadow-slate-200/50 transition-colors">
                    <button onClick={() => setActiveTab('journal')} className={`px-8 py-3 rounded-[1.4rem] text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'journal' ? 'bg-slate-900 dark:bg-sky-600 text-white shadow-xl' : 'text-slate-400'}`}>Journal</button>
                    <button onClick={() => setActiveTab('all')} className={`px-8 py-3 rounded-[1.4rem] text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'all' ? 'bg-slate-900 dark:bg-sky-600 text-white shadow-xl' : 'text-slate-400'}`}>Factures</button>
                    <button onClick={() => setActiveTab('credits')} className={`px-8 py-3 rounded-[1.4rem] text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === 'credits' ? 'bg-slate-900 dark:bg-sky-600 text-white shadow-xl' : 'text-slate-400'}`}>Avoirs</button>
                </div>
            </div>
                ...
                <div className="space-y-10 animate-in slide-in-from-bottom-4 duration-500 no-print">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                        <div className="bg-emerald-600 p-10 rounded-[2.5rem] shadow-2xl shadow-emerald-500/20 relative overflow-hidden group">
                            ...
                            <h3 className="text-4xl font-black mt-2 tracking-tighter text-white relative z-10">{totals.in.toLocaleString()} FC</h3>
                            <p className="text-[9px] font-black text-emerald-200 mt-4 uppercase opacity-60">Collectées en 24h</p>
                        </div>
                        <div className="bg-red-600 p-10 rounded-[2.5rem] shadow-2xl shadow-red-500/20 relative overflow-hidden group">
                            ...
                            <h3 className="text-4xl font-black mt-2 tracking-tighter text-white relative z-10">{totals.out.toLocaleString()} FC</h3>
                            <p className="text-[9px] font-black text-red-200 mt-4 uppercase opacity-60">Auditées GxP</p>
                        </div>
                        <div className="bg-slate-900 dark:bg-slate-800 p-10 rounded-[2.5rem] shadow-2xl relative overflow-hidden group border border-slate-800 dark:border-slate-700">
                            <div className="absolute top-0 right-0 p-8 opacity-5">
                                <svg width="100" height="100" fill="white" viewBox="0 0 24 24"><path d="M12 1v22m5-18H9.5a4.5 4.5 0 1 0 0 9h5a4.5 4.5 0 1 1 0 9H6" /></svg>
                            </div>
                            <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest relative z-10">Solde Réel Caisse</p>
                            <h3 className="text-4xl font-black mt-2 tracking-tighter text-sky-400 relative z-10">{totals.balance.toLocaleString()} FC</h3>
                            <p className="text-[9px] font-black text-slate-400 mt-4 uppercase relative z-10 tracking-widest">Liquidité Immédiate</p>
                        </div>
                    </div>
                        ...
                        <div className="bg-white dark:bg-slate-900 rounded-[3rem] border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden transition-colors">
                            <table className="w-full text-left">
                                <thead className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800">
                                    <tr>
                                        <th className="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Date / Trace ID</th>
                                        ...
                                        <tr key={m.id} className="hover:bg-slate-50/50 dark:hover:bg-slate-800 transition-colors group">
                                            <td className="px-10 py-6">
                                                <div className="flex flex-col">
                                                    <span className="text-xs font-black text-slate-800 dark:text-white uppercase">{m.id}</span>
                                                    <span className="text-[10px] text-slate-400 font-bold">{new Date(m.timestamp).toLocaleDateString()} • {new Date(m.timestamp).toLocaleTimeString()}</span>
                                                </div>
                                            </td>
                                            <td className="px-10 py-6">
                                                <span className={`px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest border ${m.type === CashMovementType.IN ? 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 border-emerald-100 dark:border-emerald-900/50' : 'bg-red-50 dark:bg-red-900/20 text-red-600 border-red-100 dark:border-red-900/50'
                                                    }`}>
                    ...
                    <div className="flex items-center gap-3 overflow-x-auto pb-4">
                        ...
                        {['ALL', 'PAID', 'PENDING', 'CANCELLED'].map(s => (
                            <button
                                ...
                                className={`px-6 py-2.5 rounded-xl text-[9px] font-black uppercase tracking-widest border transition-all ${statusFilter === s
                        ...
                        <table className="w-full text-left">
                            <thead className="bg-slate-50 dark:bg-slate-800/50 border-b border-slate-100 dark:border-slate-800">
                                <tr>
                                    ...
                                    <th className="px-10 py-6 text-[10px] font-black text-slate-400 uppercase tracking-widest">Patient</th>
                            ...
                            <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                                {filteredDocs.map(doc => (
                                    <tr key={doc.id} className="hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors group">
                                        <td className="px-10 py-7 font-black text-slate-900 dark:text-white uppercase italic tracking-tighter">{doc.id}</td>
                                        <td className="px-10 py-7">
                                            <div className="flex flex-col">
                                                <span className="font-black text-slate-800 dark:text-slate-200 text-sm">{doc.customerName}</span>
                                                <span className="text-[10px] text-slate-400 font-bold">{doc.date}</span>
                                            </div>
                                        </td>
                                        <td className="px-10 py-7 text-center">
                                            <span className={`px-5 py-2 rounded-full text-[9px] font-black uppercase tracking-widest ${doc.status === 'PAID' ? 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600' :
                                                    doc.status === 'CANCELLED' ? 'bg-red-50 dark:bg-red-900/20 text-red-600' : 'bg-amber-50 dark:bg-amber-900/20 text-amber-600'
                                                }`}>
                                                {doc.status}
                                            </span>
                                        </td>
                                        <td className="px-10 py-7 font-black text-slate-900 dark:text-sky-400 text-right">
                                            <div className="flex items-center justify-end gap-6">
                                                <span className="text-xl tracking-tighter">{doc.total.toLocaleString()} FC</span>
                ...
                <div className="fixed inset-0 z-[150] bg-slate-900/90 backdrop-blur-xl flex items-center justify-center p-4 md:p-8 overflow-auto">
                    <div className="bg-white text-slate-900 w-full max-w-3xl rounded-[2.5rem] shadow-2xl animate-in zoom-in-95 duration-200 flex flex-col min-h-[80vh]">
                        ...
                        <div className="p-8 border-b border-slate-100 flex justify-between items-center no-print bg-slate-50 rounded-t-[2.5rem]">
                            <h3 className="text-xl font-black italic uppercase tracking-tighter text-slate-800">Prévisualisation Facture</h3>
                        ...
                        <div className="p-12 print:p-0 flex-1 flex flex-col bg-white" id="invoice-printable">
                            <div className="flex justify-between items-start mb-16">
                                ...
                                <div className="text-right">
                                    ...
                                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-1">Date: {selectedDocForPrint.date}</p>
                                </div>
                            </div>

                            <div className="mb-12 grid grid-cols-2 gap-12 border-y border-slate-100 py-8">
                                <div>
                                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-3">Facturé à :</p>
                                    <h4 className="text-xl font-black text-slate-900">{selectedDocForPrint.customerName || 'Client de passage'}</h4>
                                    <p className="text-[10px] text-slate-500 font-bold uppercase tracking-widest mt-1">Réf Client: P-9982</p>
                                </div>
                                <div className="text-right">
                                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-3">Paiement :</p>
                                    <h4 className="text-lg font-black text-slate-900">{selectedDocForPrint.paymentMethod || 'Espèces'}</h4>
                                    <span className={`px-4 py-1 rounded-full text-[8px] font-black uppercase tracking-widest inline-block mt-2 ${selectedDocForPrint.status === 'PAID' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>
                            ...
                            <table className="w-full text-left mb-16">
                                ...
                                <tbody className="divide-y divide-slate-100">
                                    {selectedDocForPrint.items.length > 0 ? selectedDocForPrint.items.map((item, idx) => (
                                        <tr key={idx}>
                                            <td className="py-4 font-bold text-sm">
                                                <div className="flex flex-col">
                                                    <span>{item.name}</span>
                                                    <span className="text-[8px] text-slate-400 uppercase font-black">{item.dosage} • Lot: {item.selectedLotBatch}</span>
                                                </div>
                                            </td>
                                            <td className="py-4 text-center font-bold text-sm">x{item.quantity}</td>
                                            <td className="py-4 text-right font-bold text-sm">{item.price.toLocaleString()} FC</td>
                                            <td className="py-4 text-right font-black text-sm">{(item.price * item.quantity).toLocaleString()} FC</td>
                                        </tr>
                                    )) : (
                                        <tr>
                                            <td colSpan={4} className="py-12 text-center text-slate-300 font-bold uppercase text-[10px] tracking-widest italic">Aucun article listé (Consultez le ledger POS)</td>
                                        </tr>
                                    )}
                                </tbody>
                            </table>
                            ...
                            <div className="mt-auto flex flex-col items-end gap-3 pt-8 border-t-4 border-slate-900">
                                <div className="flex justify-between w-64 text-slate-500">
                                    ...
                                    <span className="font-bold">{selectedDocForPrint.subtotal.toLocaleString()} FC</span>
                                </div>
                                <div className="flex justify-between w-64 text-slate-500">
                                    <span className="text-[10px] font-black uppercase tracking-widest">Remise</span>
                                    <span className="font-bold">-{selectedDocForPrint.discountAmount.toLocaleString()} FC</span>
                                </div>
                                <div className="flex justify-between w-64 text-slate-900 mt-2">
                                    ...
                                    <span className="text-3xl font-black italic tracking-tighter">{selectedDocForPrint.total.toLocaleString()} FC</span>
                                </div>
                            </div>

                            <div className="mt-20 flex justify-between items-end grayscale opacity-50">
                                <div>
                                    <p className="text-[8px] font-black uppercase tracking-widest text-slate-400 mb-2">Signature & Cachet Pharmacie</p>
                                    <div className="w-40 h-16 border border-dashed border-slate-300 rounded-lg"></div>
                                </div>
                                <p className="text-[8px] font-black text-slate-400 uppercase tracking-[0.3em]">MARIASAAS • LOGICIEL DE GESTION GxP • 2024</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className="fixed inset-0 z-[110] bg-slate-900/80 backdrop-blur-xl flex items-center justify-center p-6">
                    <div className="bg-white dark:bg-slate-900 w-full max-w-xl rounded-[3.5rem] p-12 shadow-2xl animate-in zoom-in-95 duration-200 border border-slate-200 dark:border-slate-800">
                        <div className="flex justify-between items-start mb-10">
                            <div>
                                <h2 className="text-3xl font-black text-slate-900 dark:text-white italic uppercase tracking-tighter leading-none">Nouvelle Opération</h2>
                                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-2">Enregistrement Trésorerie GxP</p>
                            </div>
                            <button onClick={() => setShowAddMovement(false)} className="p-3 bg-slate-100 dark:bg-slate-800 rounded-2xl text-slate-400 hover:text-slate-900 dark:hover:text-white transition-all">
                        ...
                        <form onSubmit={handleAddMovement} className="space-y-8">
                            ...
                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-4">Catégorie de Flux</label>
                                <select
                            ...
                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-4">Montant de l'opération (FC)</label>
                                <input
                                    type="number"
                                    required
                            ...
                            <div className="space-y-2">
                                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest ml-4">Justification du Mouvement</label>
                                <textarea
                                    required
                            ...
                            <button type="submit" className="w-full py-6 bg-slate-900 dark:bg-sky-600 text-white rounded-[1.8rem] font-black uppercase text-sm tracking-[0.3em] shadow-2xl active:scale-95 transition-all mt-4">
