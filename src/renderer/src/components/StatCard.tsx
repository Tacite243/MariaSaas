...
export const StatCard: React.FC<Props> = ({ title, value, change, color, icon }) => {
    ...
    return (
        <div className="bg-white dark:bg-slate-900 p-7 rounded-[2.2rem] border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-xl hover:shadow-emerald-500/5 transition-all duration-300 group">
            <div className="flex justify-between items-start mb-6">
                <div className={`p-3 rounded-2xl ${themes[color]} transition-transform group-hover:scale-110`}>
                    <svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" d={icon} />
                    </svg>
                </div>
                <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${themes[color]}`}>
                    {change}
                </span>
            </div>
            <div>
