       ...
       <div className="h-60 w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data}>
               ...
               <YAxis hide />
               <Tooltip cursor={{fill: 'transparent'}} contentStyle={{backgroundColor: isDark ? '#0f172a' : '#fff', borderRadius: '10px', border: 'none', color: isDark ? '#fff' : '#000'}} />
               <Bar dataKey="sales" radius={[8, 8, 0, 0]} barSize={30}>
                  {data.map((entry, index) => (
                    // La dernière barre (aujourd'hui) est colorée
                    <Cell key={`cell-${index}`} fill={index === data.length - 1 ? activeColor : (isDark ? '#e2e8f0' : '#d1fae5')} className="hover:opacity-80 transition-opacity dark:fill-slate-800" />
                  ))}
               </Bar>
            </BarChart>
          </ResponsiveContainer>
